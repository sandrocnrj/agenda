...
if ( isset( $_POST ) && ! empty( $_POST ) ) {
	...
	if ( ! $erro ) {
		...
		if ( ! empty( $user_id ) ) {
			...
			$inserir_usuario->execute( array( $form_usuario,  crypt( $form_senha ), $form_nome, $user_id ) );
		...
		// Se o usuário não existir, cadastra novo
			...
			$inserir_usuario->execute( array( $form_usuario, crypt( $form_senha ), $form_nome ) );
		}
	}
}
?>
...
<?php include_once 'header.php'; ?>
		<div class="container">
			<div>
				<hr><p>Cadastra ou edita usuário já cadastrado.</p><hr>
			</div>
			<form action="" method="post">
			<table>
				<tr>
					<td>Nome do Usuário<br><input type="text" name="form_nome"></td><td></td>
					<td>Usuário<br><input type="text" name="form_usuario" required></td><td></td>
					<td>Senha<br><input type="password" name="form_senha" required></td>
				</tr>
				
				<?php if ( ! empty ( $erro ) ) :?>
					<tr>
						<td style="color: red;"><div class='alert alert-info'>
					<i class="fa fa-group"></i> <strong><?php echo $erro; ?>!</strong>
				</div></td>
					</tr>
				<?php endif; ?>
		...
		<?php if($_SESSION['logado'] === true) { ?>
			<div class="container">
				<div class='alert alert-info'>
					<i class="fa fa-group"></i> <strong>Usuários Cadastrado!</strong>
				</div>
